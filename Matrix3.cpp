...
void operacii_nad_matricey() {       //функция операций над матрицей
	...
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < M; j++) {
			if (arr[i][j] / 10 >= 0 && arr[i][j] / 10 < 10) {            //поиск 2-значных элементов матрицы и занесение их массив
				dva_znaka_arr[kolichestvo_dva_znaka] = arr[i][j];        //для 2-значных элементов
				kolichestvo_dva_znaka++;
			}
			else if (arr[i][j] / 10 >= 10 && arr[i][j] / 10 < 100) {     //поиск 3-значных элементов матрицы и занесение их массив
